...
for name in ["SubscriptionOfferPhase", "RegionalSubscriptionOfferConfig", "OtherRegionsSubscriptionOfferConfig",
             "SubscriptionOfferTargeting", "OfferTargetingRule", "OfferPhase"]:
    if name in sc:
        d = sc[name]
        print("=" * 8, name, "=" * 8)
        print((d.get("description") or "")[:100])
        for k, v in d.get("properties", {}).items():
            ref = v.get("$ref", v.get("type", ""))
            print("   ", k, "->", ref, (v.get("description") or "")[:60])
